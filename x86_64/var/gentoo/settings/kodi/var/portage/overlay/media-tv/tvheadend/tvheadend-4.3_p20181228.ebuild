...
GIT_COMMIT="b17dcf9"
SRC_URI="https://github.com/tvheadend/tvheadend/tarball/${GIT_COMMIT} -> ${P}.tar.gz"
S=${WORKDIR}/tvheadend-tvheadend-${GIT_COMMIT}
...
PATCHES=(
	...
	"${FILESDIR}"/disable-dvb.patch
	"${FILESDIR}"/disable-dvb-2.patch
)
...
src_configure() {
	sed -i s/0.0.0~unknown/${PV}/ ${S}/support/version || die
	CC="$(tc-getCC)" \
